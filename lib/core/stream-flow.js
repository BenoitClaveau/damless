...
class StreamFlow extends PassThrough {
    ...
    _read(n) {
        if (this.backpressuring) {
            this.backpressuring = false;
            console.log("** READ ALL INNER BUFFER")
            while (this.inner.read() !== null) { // we read all data in inner buffer (before sending the drain event)
            }
            console.log("** CALL REUME")
            this.inner.once("resume", () => {
                console.log("** RESUMED")
                this.emit("drain"); // ask to drain.
            });
            this.inner.resume();
        }
        else
    ...
    write(chunk, encoding, callback) {
        ...
        if (this.backpressuring) console.log("back pressure");
