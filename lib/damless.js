...
class DamLess {
    constructor(injector, config, eventEmitter) {
    ...
    async mount() {
        this.router = await this.injector.resolve("http-router"); // Create http-router and isitasset, isitget... before resolve assets & routes.
    ...
    async unmount() {
        if (this.httpServer) {
            ...
            this.eventEmitter.emit("close", { server: "http", port: this.config.http.port });
        }
        if (this.httpsServer) {
            await new Promise((resolve, reject) => this.httpsServer.close(resolve));
            this.eventEmitter.emit("close", { server: "https", port: this.config.https.port });
        }
        if (this.http2SecureServer) {
            await new Promise((resolve, reject) => this.http2SecureServer.close(resolve));
            this.eventEmitter.emit("close", { server: "http2", port: this.config.http2.port });
        }
        if (this.http2Server) {
            await new Promise((resolve, reject) => this.http2Server.close(resolve));
            this.eventEmitter.emit("close", { server: "http2-unsecure", port: this.config["http2-unsecure"].port });
        }
    }
    ...
    async invoke(request, response) {
        try {
            ...
            if (!error.context) error.context = { method, url };
    ...
    async invoke2(stream, headers, flags) {
        try {
            ...
            return await this.router.invoke(context, wrappedStream, headers, flags);
        }
        catch (error) {
            if (!response.headersSent) {
                response.statusCode = error.statusCode || 500;
            ...
            if (!error.context) error.context = { method, url };
            this.eventEmitter.emit("error", error);  // TODO set context
        }
    };
    ...
    async createHttpServer(config) {
        ...
        this.eventEmitter.emit("listening", { server: "http", port: config.port });
    }
    ...
    async createHttpsServer(config) {
        ...
        this.eventEmitter.emit("listening", { server: "https", port: config.port });
    }
    ...
    async createHttp2SecureServer(config) {
        ...
        this.eventEmitter.emit("listening", { server: "http2", port: config.port });
    }
    ...
    async createHttp2Server(config) {
        ...
        this.eventEmitter.emit("listening", { server: "http-unsecure", port: config.port });
    }
};

exports = module.exports = DamLess;