...
class HttpServer extends EventEmitter {
    ...
    async invoke(request, response) {
        const handlerError = error => this.handledError(error, { response });
        try {
            const router = await this.injector.resolve("http-router");
            ...
            try {
                stream.once("error", handlerError); // if stream emit an error. Normally not because it's better tu use async pipeline.
                await router.invoke(context, stream, headers);
            }
            catch (error) {
                // Je recréé le stream car il a du être détruit (destroy by pipeline)
                const stream = await factory.getStream(request, response, headers);
                stream.once("error", handlerError); // if writeError emit an error via stream. Normally not because it's better tu use async pipeline.
    ...
    async invoke2(stream, headers, flags) {
        const handlerError = error => this.handledError(error, { stream });
        try {
            const router = await this.injector.resolve("http-router");
            const factory = await this.injector.resolve("context-factory");
            const {
            ...
            try {
                wrappedStream.once("error", handlerError); // if stream emit an error. Normally not because it's better tu use async pipeline.
                ...
                const wrappedStream = await factory.getStream(stream, stream, headers);
                stream.once("error", handlerError); // if writeError emit an error via stream. Normally not because it's better tu use async pipeline.
                await this.writeError(error, context, wrappedStream, headers, flags);
            }
        }
        catch (error) {
            handlerError(error);
    ...
    handledError(error, options = {}) {
        const { response, stream } = options;
        const statusCode = error.statusCode || 500;
        if (response && !response.headersSent) { // http 1
            response.statusCode = statusCode;
            response.end();
            return;
        }
        if (stream && !stream.headersSent) { // http 2
            stream.respond({ ':status': statusCode });
            stream.end();
            return;
        }
        console.error("Http-server handle error", error.stack ? error.stack : error.message);
    }
    ...
    async writeError(error, context, stream, headers) {
        ...
        if (stream.headersSent) {
            ...
            //stream.end();
            console.error("Write error in trailer because headers has been sent", error.stack ? error.stack : error.message);
            return;
        }
        ...
        if (statusCode == 404)
            stream
                ...
                .end({ message: `Page ${context.pathname} not found.`, stack });
        else {
            console.error("Write error", error.stack ? error.stack : error.message);
            stream
                .mode("object")
                .respond({
                    statusCode,
