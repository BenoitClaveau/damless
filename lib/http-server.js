...
//         console.log(`${entry.duration}  ${entry.name}`);
//     })
// })
...
class HttpServer extends EventEmitter {
            ...
            this.httpServer.on("request", async (request, response) => {
                try {
                    ...
                    response.on("finish", () => {
                        ...
                        performance.measure(request.url, 'request-start', 'request-end');
                    })
                    response.on("error", error => {
                        console.error(new Date().toLocaleString('fr-FR'), `NODE.JS RESPONSE ERROR ${request.method}:${request.url}`, error);
                    })
                    if (config?.request?.timeout) request.setTimeout(config.request.timeout);
                    if (config?.response?.timeout) response.setTimeout(config.response.timeout);
                    this.invoke(request, response);
                }
                catch (error) {
                    console.error(new Date().toLocaleString('fr-FR'), `FATAL ERROR`, error);
                    process.exit(-1)
                }
            });
    ...
    async createHttp2Server(config) {
        ...
        this.http2Server.on("stream", (stream, headers, flags) => {
            try {
                // if (config?.request.timeout) stream.setTimeout(config?.request.timeout);
                // if (config?.response.timeout) stream.setTimeout(config?.response.timeout);
                this.invoke2(stream, headers, flags);
            }
            catch (error) {
                console.error(new Date().toLocaleString('fr-FR'), `FATAL ERROR`, error);
