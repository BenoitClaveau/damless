...
const { streamify } = require("./streams/streamify");
...
class HttpServer extends EventEmitter {
    ...
    handleError(error, context, stream, headers, flags) {
        ...
        if (!stream.headersSent) {
            ...
            // if header already sent (chunk already succeeded), we add a trailer to inform user and "end" the response.
            stream.addTrailers({
                status: statusCode,
                error: message,
            }).end();
    ...
    async invoke(request, response) {
            ...
            d.on('error', error => this.handleError(error, context, stream, headers));
            d.add(request);
            d.add(response);
            d.add(stream);
    ...
    async invoke2(stream, headers, flags) {
        ...
        const d = domain.create();
        d.on('error', error => this.handleError(error, context, wrappedStream, headers, flags));
        d.add(stream);
        d.add(wrappedStream);
