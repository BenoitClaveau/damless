...
class HttpServer extends EventEmitter {
    ...
    async invoke(request, response) {
        ...
        try {
            ...
            // // cas des pipeline avec async/await. stream peut-être close à cause du pipeline, mais l'erreur http pas encore envoyée (response).
            // // Je vais écouter les erreurs. pour stopper la requête (A VERIFIER).
            // if (streamClosed && !responseClosed) {
            ...
            // }
            // // cas des pipeline (non async/await)
            // else if (!streamClosed && !responseClosed) {

            //     // stream est clos. La réponse d'erreur http aurai du être envoyée
            //     // throw new Error(`Response has not be closed after treatment.`);
            // }
        }
        catch (error) {
