...
class HttpServer extends EventEmitter {
        ...
        try {
            ...
            try {
                stream.once("error", error => {
                    stream.once("error", error => this.handledError(error, { response }))
                    this.writeError(error, context, stream, headers);
                });
            ...
            this.handledError(error, { response })
        }
    };
    ...
    async invoke2(stream, headers, flags) {
        try {
            ...
            try {
                wrappedStream.once("error", error => {
                    wrappedStream.once("error", error => this.handledError(error, { stream }))
                    this.writeError(error, context, wrappedStream, headers, flags);
                });
            ...
            this.handledError(error, { stream })
        }
    }
