...
class HttpServer extends EventEmitter {
    ...
    async asset(route, filepath, options) {
        try {
            ...
            // Manage import without extension for javascript file
            if (item.headers["Content-Type"] == "application/javascript") {
                const { dir, name } = path.parse(route);
                const routeWithoutExt = `${dir}/${name}`;
                if (routeWithoutExt != route) {
                    const item = await router.asset(routeWithoutExt);
                    await item.init(filepath, options, item.headers);
                }  
            }
