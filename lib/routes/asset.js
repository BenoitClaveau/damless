...
class Asset extends Get {
    ...
    mount() {
        super();
        this.contentType = await this.giveme.resolve("content-type");
    }
    ...
    async init(filepath, options) {
        ...
        try {
            this.contentType = this.contentType.get(this.file.extname);
        }
        catch(error) {
    ...
    async invoke (context, stream, headers) {
        ...
        this.damless.emit("asset", { context, file, headers: responseHeaders });
