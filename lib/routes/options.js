...
class Options {
    ...
    async invoke(context, stream, headers) {
        ...
        const responseHeaders = { "Allow": allow.join() };

        // const eventEmitter = await this.giveme.resolve("eventEmitter");
        // eventEmitter.emit("invoke", { context, headers: responseHeaders }); //TO set headers
