...
class AskReply extends PassThrough {
    ...
    constructor(giveme, request, response, headers) {
        ...
        /* writable event */
        this.on("close", () => {
            console.log("[ASK-REPLY] on close")
        });
        this.once("end", () => {
            console.log("[ASK-REPLY] on end")
        });
        this.once("finish", () => {
            console.log("[ASK-REPLY] on finish")
        });
    ...
    onceWritableInit(chunk, encoding) {
        ...
        this[kWritableStreamCurrent]
            .on("data", data => {
                console.log("[kWritableStreamCurrent][data]", data.toString())
        ...
        pump(this[kWritableStreamCurrent], this[kResponse])
            .on("finish", () => {
                console.log("[kResponse][finish] emit finish");
                this.emit("finish");
            })
            .on("close", () => {
                console.log("[kResponse][close] emit close");
                this.emit("close");
            });
    ...
    //override end to not emit finish. It will be emitted after (via response)
    end(chunk, encoding, callback) {
        console.log("[ASK-REPLY][end]");
