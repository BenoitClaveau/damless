...
class AskReply extends PassThrough {
    ...
    constructor(giveme, request, response, headers) {
        ...
        this.once("error", error => this.emit("error", error));
    }
    ...
    async redirect(url) {
        ...
        const completed = ending(this, { resume: false }); // wait stream ending to catch error.
        ...
        await completed; // wait ending to catch all error.
    };
