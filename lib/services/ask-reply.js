...
class AskReply extends PassThrough {
    ...
    async mount() {
        this[kJsonStream] = await this[kGiveme].resolve("json-stream");
    }
    ...
    async forward(method, url, headers) {
        const httpRouter = await this[kGiveme].resolve("http-router");
        const contextFactory = await this[kGiveme].resolve("context-factory");
    ...
    writeError(error) {
        ...
        if (!this[kResponse].headersSent) {
            ...
            if (statusCode == 404)
                ...
                if (!data) this.respond({ statusCode }).end({ message, stack });
                else {
                    const {
                        stream,
                        headers,
                    ...
                    this.respond({ statusCode }).end({ message, stack, data: otherData });
                }
            }
        }
    }
