...
class AskReply extends Duplex {
    ...
    constructor(giveme, request, response, headers) {
        ...
        this.once("writableinit", () => { // create pipeline to write the response
            pipeline(
                ...
                this.compress
                    .once("data", data => this.writeHead(data))
                    .once("end", () => this.writeHead())
                ,
                this[kResponse],
                error => {
                    if (error) {
                        console.error("Error in writable pipeline", error.stack ? error.stack : error.message);
                    }
                }
            );
        });
    ...
    writeHead(data) {
        if (!this[kResponse].headersSent) {
            if (this[kResponse] instanceof OutgoingMessage) {
                ...
                console.log("writeHead during ondata. statusCode:", this[kStatusCode], "data:", data)
            }
            else {
    ...
    abort(error) {
        ...
        //this[kRequest].connection.destroy();
        this[kResponse].connection.destroy(error);
    }
};
