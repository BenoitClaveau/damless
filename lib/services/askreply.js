        ...
        return function* () {
            ...
            yield new TransformWrapper(function* () {
                ...
                if (/\bapplication\/json\b/.test(mime)) {
                    ...
                    yield this.jsonStream.parse({ isArray, source: "ask" })
                }
        ...
        return function* () {
            yield new Transform({
                async transform(chunk, encoding, callback) {
                    try {
                        if (detect) {
                            await this.detectContentType(chunk);
                            detect = true
                        }
                        callback(null, chunk);
                    }
            ...
            yield new TransformWrapper(function* () {
                ...
                if (/\bapplication\/json\b/.test(contentType)) {
                    ...
                    yield this.jsonStream.stringify({ isArray, source: "reply" });
                }
                if (/\btext\/html\b/.test(contentType)) {
                    const isArray = this.writableMode === "array";
                    yield this.textStream.stringify({ isArray, source: "reply" });
                }
