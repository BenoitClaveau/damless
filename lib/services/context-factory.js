...
class ContextFactory {
    ...
    async getStream(request, response, headers, options = {}) {
        // par défaut readableObjectMode = true, je privilégie JSON.
        let { readableObjectMode = true, writableObjectMode } = options;
        const { AskReply, giveme } = this;
        // gestion des modes pour le stream askreply
        if (options.objectMode !== undefined) readableObjectMode = writableObjectMode = options.objectMode;
        const askReply = new AskReply(giveme, request, response, headers, { readableObjectMode, writableObjectMode });
