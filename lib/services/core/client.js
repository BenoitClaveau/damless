...
class ClientService {
    ...
    createError(httpResponse) {
        let { statusCode, headers, body: { message, stack }, body } = httpResponse;
        statusCode = statusCode || 500;
        if (/application\/json/.test(headers["content-type"]) && typeof body === "string") body = this.json.parse(body);
        if (message && stack)
           return new HttpError(statusCode, "\nstatusCode: ${statusCode},\nmessage: ${message},\nstack: ${stack}", { statusCode, headers, message, stack });
        if (message)
            return new HttpError(statusCode, "\nstatusCode: ${statusCode},\nmessage: ${message}", { statusCode, headers, message });
        if (typeof body === "string") 
            return new HttpError(statusCode, `\nstatusCode: ${statusCode},\nbody: ${body}`, { statusCode, headers, body });
        return new HttpError(statusCode, "\nstatusCode: ${statusCode}", { statusCode, headers, body });
    }
