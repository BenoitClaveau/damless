...
class Cors {
    ...
    constructor() {
        this.options = {
            window: 60, // `window` of 60s
            max: 60, // max number of recent connections during `window` milliseconds before sending a 429 status = Too Many Requests (RFC 6585)
            frame: 5
        }
        this.hits = {
            // counter: 
    ...
    async invoke(context, stream, headers) {
        ...
        if (incr > this.options.max) {
            stream.headers = ({
                ...stream.headers,
                statusCode: 429,
                "retry-after": Math.ceil(this.options.window),
            });
            return stream.end();
        }
    };
